...
<header class="entry-header">
   <h1 class="entry-title"><?php echo get_Project_Title($id) ?> - Home Building - <?php echo ucwords($action) ?></h1>
</header>
<div class="entry-content">
<?php

         // replace table name  by your table name
         $table_name = 'home_building';
         ...
         switch($action) {
            ...
            case "delete":
               ...
               $stmt = $sys->db()->prepare("DELETE FROM [dbo].[" . $table_name . "] WHERE hb_id = :hb_id;");
               $stmt->bindParam(":hb_id", $id);
            ...
            case "add":
               ...
               if (ISSET($_POST['submit-action'])) {
                  if ($_POST['submit-action'] == "add") {
                     ...
                     $sql .= ":hb_id,:hb_last_modified,";

                     foreach ($_POST as $key => $value) {
                        if ($key != "submit-action" && $key != "hb_last_modified" && $key != "hb_id") {
                     ...
                     $stmt->bindValue(":hb_id", $id, PDO::PARAM_INT);
                     $stmt->bindValue(":hb_last_modified", time(), PDO::PARAM_INT);

                     foreach ($_POST as $key => $value) {

                        if ($key != "submit-action" && $key != "hb_id") {
                     ...
                     LogActivity($sys->auth()->user_name, "INSERT HB_ID: " . $id);
            ...
            case "edit":
               if (ISSET($_POST['submit-action'])) {
                  if ($_POST['submit-action'] == "edit") {
                     ...
                     for ($i = 0; $i < count($table_fields); $i++) {
                        if ($table_fields[$i]["COLUMN_NAME"] != "hb_id") {

                           if ($table_fields[$i]["COLUMN_NAME"] == "hb_last_modified") {
                     ...
                     LogActivity($sys->auth()->user_name, "UPDATE HB_ID: " . $id);
            ...
            default:
              ...
              //SQL Statement
              $stmt = $sys->db()->prepare("SELECT * FROM [dbo].[" . $table_name . "] WHERE hb_id = :hb_id;", array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
              $stmt->bindParam(":hb_id", $id);
              ...
              for ($i = 0; $i < count($table_fields); $i++) {
                 ...
                 switch (strtolower($row_type)) {
                    ...
                    case "hb_id-view":
                       ...
                       break;
                    case "hb_last_modified-view":
                    case "hb_last_modified-edit":
                       echo $row_div . "\n";
                       echo $label_div . ucwords($map_hb[$table_fields[$i]["COLUMN_NAME"]]) . $close_div . "\n";
                    ...
                    case "hb_owner_contact-edit":
                    case "hb_team1-edit":
                    case "hb_team2-edit":
                    case "hb_team3-edit":
                    case "hb_team4-edit":
                    case "hb_engi_contact-edit":
                    case "hb_exc_contact-edit":
                    case "hb_wet_contact-edit":
                    case "hb_pavi_contact-edit":
                    case "hb_dry_contact-edit":
                    case "hb_sswcr1-edit":
                    case "hb_sswcr2-edit":
                    case "hb_dswcr-edit":
                       echo $row_div . "\n";
                       ...
                       echo "\t" . $close_div . "\n";
                       break;
                    case "hb_owner_contact_company-edit":
                    case "hb_exc_contact_company-edit":
                    case "hb_wet_contact_company-edit":
                    case "hb_pavi_contact_company-edit":
                    case "hb_dry_contact_company-edit":
                    case "hb_engi_contact_company-edit":
                       echo $row_div . "\n";
                          echo $label_div . ucwords($map_hb[$table_fields[$i]["COLUMN_NAME"]]) . $close_div . "\n";
                       ...
                       echo "\t" . $close_div . "\n";
                       break;
                    case "hb_soil1_type-edit":
                    case "hb_soil2_type-edit":
                    case "hb_soil3_type-edit":
                    case "hb_soil4_type-edit":
                    case "hb_soil5_type-edit":
                    case "hb_soil6_type-edit":
                    case "hb_soil7_type-edit":
                       echo $row_div . "\n";
                          echo $label_div . ucwords($map_hb[$table_fields[$i]["COLUMN_NAME"]]) . $close_div . "\n";
