...
<header class="entry-header">
   <h1 class="entry-title"><?php echo get_Project_Title($id) ?> - General Contracting - <?php echo ucwords($action) ?></h1>
</header>
<div class="entry-content">
<?php

         // replace table name  by your table name
         $table_name = 'gen_contracting';
         ...
         switch($action) {
            ...
            case "delete":
               ...
               $stmt = $sys->db()->prepare("DELETE FROM [dbo].[" . $table_name . "] WHERE gc_id = :gc_id;");
               $stmt->bindParam(":gc_id", $id);

               $stmt->execute();

               LogActivity($sys->auth()->user_name, "DELETE GC_ID: " . $id);
            ...
            case "add":
               ...
               if (ISSET($_POST['submit-action'])) {
                  if ($_POST['submit-action'] == "add") {
                     ...
                     $sql .= ":gc_id,:gc_last_modified,";

                     foreach ($_POST as $key => $value) {
                        if ($key != "submit-action" && $key != "gc_last_modified" && $key != "gc_id") {
                     ...
                     $stmt->bindValue(":gc_id", $id, PDO::PARAM_INT);
                     $stmt->bindValue(":gc_last_modified", time(), PDO::PARAM_INT);

                     foreach ($_POST as $key => $value) {

                        if ($key != "submit-action" && $key != "gc_id") {
                     ...
                     LogActivity($sys->auth()->user_name, "INSERT GC_ID: " . $id);
            ...
            case "edit":
               if (ISSET($_POST['submit-action'])) {
                  if ($_POST['submit-action'] == "edit") {
                     ...
                     for ($i = 0; $i < count($table_fields); $i++) {
                        if ($table_fields[$i]["COLUMN_NAME"] != "gc_id") {

                           if ($table_fields[$i]["COLUMN_NAME"] == "gc_last_modified") {
                     ...
                     LogActivity($sys->auth()->user_name, "UPDATE GC_ID: " . $id);
            ...
            default:
              ...
              //SQL Statement
              $stmt = $sys->db()->prepare("SELECT * FROM [dbo].[" . $table_name . "] WHERE gc_id = :gc_id;", array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
              $stmt->bindParam(":gc_id", $id);
              ...
              for ($i = 0; $i < count($table_fields); $i++) {
                 ...
                 switch (strtolower($row_type)) {
                    ...
                    case "gc_id-view":
                       ...
                       break;
                    case "gc_last_modified-view":
                    case "gc_last_modified-edit":
                       echo $row_div . "\n";
                       echo $label_div . ucwords($map_gc[$table_fields[$i]["COLUMN_NAME"]]) . $close_div . "\n";
                    ...
                    case "gc_owner_contact-edit":
                    case "gc_team1-edit":
                    case "gc_team2-edit":
                    case "gc_team3-edit":
                    case "gc_team4-edit":
                    case "gc_engi_contact-edit":
                    case "gc_exc_contact-edit":
                    case "gc_wet_contact-edit":
                    case "gc_pavi_contact-edit":
                    case "gc_dry_contact-edit":
                       echo $row_div . "\n";
                          echo $label_div . ucwords($map_gc[$table_fields[$i]["COLUMN_NAME"]]) . $close_div . "\n";
                       ...
                       echo "\t" . $close_div . "\n";
                       break;
                    case "gc_owner_contact_company-edit":
                    case "gc_exc_contact_company-edit":
                    case "gc_wet_contact_company-edit":
                    case "gc_pavi_contact_company-edit":
                    case "gc_dry_contact_company-edit":
                    case "gc_engi_contact_company-edit":
                       echo $row_div . "\n";
                          echo $label_div . ucwords($map_gc[$table_fields[$i]["COLUMN_NAME"]]) . $close_div . "\n";
                       ...
                       echo "\t" . $close_div . "\n";
                       break;
                    case "gc_soil1_type-edit":
                    case "gc_soil2_type-edit":
                    case "gc_soil3_type-edit":
                    case "gc_soil4_type-edit":
                    case "gc_soil5_type-edit":
                    case "gc_soil6_type-edit":
                    case "gc_soil7_type-edit":
                       echo $row_div . "\n";
                          echo $label_div . ucwords($map_gc[$table_fields[$i]["COLUMN_NAME"]]) . $close_div . "\n";
