...
<header class="entry-header">
   <h1 class="entry-title">Endangered Species - <?php echo ucwords($action) ?></h1>
</header>
...
<?php
         ...
         switch($action) {
            ...
            case "delete":
               ...
               $stmt = $sys->db()->prepare("DELETE FROM [dbo].[" . $table_name . "] WHERE es_id = :es_id;");
               $stmt->bindParam(":es_id", $id);

               $stmt->execute();

               LogActivity($sys->auth()->user_name, "DELETE ES_ID: " . $id);
               ...
               if (ISSET($_POST['submit-action'])) {
                  if ($_POST['submit-action'] == "add") {
                     ...
                     for ($i = 0; $i < count($table_fields); $i++) {
                        if (strtolower($table_fields[$i]["COLUMN_NAME"]) != "es_id") {
                     ...
                     $sql .= ":es_last_modified,";

                     foreach ($_POST as $key => $value) {
                        if ($key != "submit-action" && $key != "es_id") {
                     ...
                     $stmt->bindValue(":es_last_modified", time(), PDO::PARAM_INT);
                     ...
                     LogActivity($sys->auth()->user_name, "INSERT ES_ID: " . $id);
            ...
            case "edit":
               if (ISSET($_POST['submit-action'])) {
                  if ($_POST['submit-action'] == "edit") {
                     ...
                     for ($i = 0; $i < count($table_fields); $i++) {
                        if ($table_fields[$i]["COLUMN_NAME"] != "es_id") {

                           if ($table_fields[$i]["COLUMN_NAME"] == "es_last_modified") {
                     ...
                     $stmt->bindValue(":es_id", $id, PDO::PARAM_INT);

                     $stmt->execute();

                     LogActivity($sys->auth()->user_name, "UPDATE ES_ID: " . $id);
            ...
            default:
              ...
              //SQL Statement
              $stmt = $sys->db()->prepare("SELECT * FROM [dbo].[" . $table_name . "] WHERE es_id = :es_id;", array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
              $stmt->bindParam(":es_id", $id);
              ...
              for ($i = 0; $i < count($table_fields); $i++) {
                 ...
                 switch (strtolower($row_type)) {
                    ...
                    case "es_id-view":
                       ...
                       break;
                    case "es_last_modified-view":
                    case "es_last_modified-edit":
                       echo $row_div . "\n";
                       echo $label_div . ucwords($map_es[$table_fields[$i]["COLUMN_NAME"]]) . $close_div . "\n";
