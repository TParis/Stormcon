...
<header class="entry-header">
   <h1 class="entry-title"><?php echo get_Project_Title($id) ?> - Gas - <?php echo ucwords($action) ?></h1>
</header>
...
<?php
         ...
         switch($action) {
            ...
            case "delete":
               ...
               $stmt = $sys->db()->prepare("DELETE FROM [dbo].[" . $table_name . "] WHERE gas_id = :gas_id;");
               $stmt->bindParam(":gas_id", $id);

               $stmt->execute();

               LogActivity($sys->auth()->user_name, "DELETE gas_ID: " . $id);
            ...
            case "add":
               ...
               if (ISSET($_POST['submit-action'])) {
                  if ($_POST['submit-action'] == "add") {
                     ...
                     $sql .= ":gas_id,:gas_last_modified,";

                     foreach ($_POST as $key => $value) {
                        if ($key != "submit-action" && $key != "gas_last_modified" && $key != "gas_id") {
                     ...
                     $stmt->bindValue(":gas_id", $id, PDO::PARAM_INT);
                     $stmt->bindValue(":gas_last_modified", time(), PDO::PARAM_INT);

                     foreach ($_POST as $key => $value) {

                        if ($key != "submit-action" && $key != "gas_id") {
                     ...
                     LogActivity($sys->auth()->user_name, "INSERT gas_ID: " . $id);
            ...
            case "edit":
               if (ISSET($_POST['submit-action'])) {
                  if ($_POST['submit-action'] == "edit") {
                     ...
                     for ($i = 0; $i < count($table_fields); $i++) {
                        if ($table_fields[$i]["COLUMN_NAME"] != "gas_id") {

                           if ($table_fields[$i]["COLUMN_NAME"] == "gas_last_modified") {
                     ...
                     LogActivity($sys->auth()->user_name, "UPDATE gas_ID: " . $id);
            ...
            default:
              ...
              //SQL Statement
              $stmt = $sys->db()->prepare("SELECT * FROM [dbo].[" . $table_name . "] WHERE gas_id = :gas_id;", array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
              $stmt->bindParam(":gas_id", $id);
              ...
              for ($i = 0; $i < count($table_fields); $i++) {
                 ...
                 switch (strtolower($row_type)) {
                    ...
                    case "gas_id-view":
                       ...
                       break;
                    case "gas_last_modified-view":
                    case "gas_last_modified-edit":
                       echo $row_div . "\n";
                       echo $label_div . ucwords($map_gas[$table_fields[$i]["COLUMN_NAME"]]) . $close_div . "\n";
                    ...
                    case "gas_owner_contact-edit":
                    case "gas_team1_name-edit":
                    case "gas_team2_name-edit":
                    case "gas_team3_name-edit":
                    case "gas_team4_name-edit":
                    case "gas_engi_contact-edit":
                    case "gas_exc_contact-edit":
                    case "gas_wet_contact-edit":
                    case "gas_pavi_contact-edit":
                    case "gas_dry_contact-edit":
                       echo $row_div . "\n";
                          echo $label_div . ucwords($map_gas[$table_fields[$i]["COLUMN_NAME"]]) . $close_div . "\n";
                       ...
                       echo "\t" . $close_div . "\n";
                       break;
                    case "gas_owner_contact_company-edit":
                    case "gas_exc_contact_company-edit":
                    case "gas_wet_contact_company-edit":
                    case "gas_pavi_contact_company-edit":
                    case "gas_dry_contact_company-edit":
                    case "gas_engi_contact_company-edit":
                       echo $row_div . "\n";
                          echo $label_div . ucwords($map_gas[$table_fields[$i]["COLUMN_NAME"]]) . $close_div . "\n";
                       ...
                       echo "\t" . $close_div . "\n";
                       break;
                    case "gas_soil1_type-edit":
                    case "gas_soil2_type-edit":
                    case "gas_soil3_type-edit":
                    case "gas_soil4_type-edit":
                    case "gas_soil5_type-edit":
                    case "gas_soil6_type-edit":
                    case "gas_soil7_type-edit":
                       echo $row_div . "\n";
                          echo $label_div . ucwords($map_gas[$table_fields[$i]["COLUMN_NAME"]]) . $close_div . "\n";
